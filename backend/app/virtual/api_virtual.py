...
- لا تستخدم قوالب جاهزة أو عبارات مكررة، بل اجعل الحكم خاصًا بالقضية.
 ...
 يجب أن يشمل الحكم على علامات الاقتباسات ونقاط الترقيم:

- مقدمة شرعية رسمية **(يمكن وضعها بعلامة ## المقدمة)**  
- ملخص موجز للوقائع والأدلة، مكتوب بنقاط مرقمة أو فقرات قصيرة **(## ملخص الوقائع والأدلة)**  
- الحكم النهائي مُقسم إلى فقرات مرقمة (أولًا، ثانيًا، ثالثًا...)  
- **اقتباسات شرعية** (آية، حديث، مادة نظامية)، تُوضع داخل كتل اقتباس (Markdown blockquote)  
- إبراز أهم النقاط باستخدام علامات التسطير أو العلامة *  
- خاتمة رسمية

أمثلة لعلامات Markdown:  
- `##` للعناوين  
- `*` أو `-` للقوائم  
- `> ` للاقتباسات  
- **نص عريض** للنقاط الهامة  
- فاصل `---` بين الأقسام
...
# Phase 2: Build similar cases summaries
    ...
    final_result = generate_final_judgment(
        model["phase2"],
        case_input=user_query,        # The user’s detailed case as input
